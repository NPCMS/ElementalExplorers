...
public class AtlasTexturesNode : SyncExtendedNode
{
	...
	public override IEnumerator CalculateOutputs(Action<bool> callback)
	{
		...
		foreach (KeyValuePair<Material,List<MeshRenderer>> instance in renderers)
		{
			...
			for (int i = 0; i < texIDs.Length; i++)
			{
				Texture2D tex = (Texture2D)material.GetTexture(texIDs[i]);
				// Debug.Assert(tex == null,texIDs[i] + " "+ material.name);
				// Debug.Assert(tex.isReadable, "Not readable: " + material.name + " " + tex.name);
				textures[i].Add(tex);
			}
		}
