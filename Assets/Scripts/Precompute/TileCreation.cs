...
public class TileCreation : MonoBehaviour
{
    ...
    Vector2Int GetTileFromCoord(double longitude, double latitude, float level = 12)
    {
    ...
    double LongitudeFromTile(Vector2Int tile, int n)
    {
        return 360.0 * tile.x / n - 180.0;
    }

    double LatitudeFromTile(Vector2Int tile, int n)
    {
        double m = Math.PI - 2.0 * Math.PI * tile.y / n;
        return (180.0 / Math.PI) * Math.Atan(0.5 * (Math.Exp(m) - Math.Exp(-m)));
    }

    GlobeBoundingBox GetBoundingBoxFromTile(Vector2Int tile, float level = 12)
    {
        int n = (int) Math.Pow(2, level);
        double south = LatitudeFromTile(tile, n);
        double west = LongitudeFromTile(tile, n);
        double north = LatitudeFromTile(tile + Vector2Int.up, n);
        double east = LongitudeFromTile(tile + Vector2Int.right, n);
